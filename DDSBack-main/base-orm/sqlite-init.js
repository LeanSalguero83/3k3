...
async function CrearBaseSiNoExiste() {
  ...
  let existeHoteles = false;
  let resHoteles = null;
  existeHoteles = false;

  sqlHoteles =
    "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'hoteles'";
  resHoteles = await db.get(sqlHoteles, []);
  if (resHoteles.contar > 0) existeHoteles = true;

  // para pacientes

  let existePacientes = false;
  let resPacientes = null;
  existePacientes = false;

  sqlPacientes =
    "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'pacientes'";
  resPacientes = await db.get(sqlPacientes, []);
  if (resPacientes.contar > 0) existePacientes = true;
  ...
  if (!existeHoteles) {
    ...
    await db.run(`INSERT INTO hoteles (NombreFantasia, Habitaciones)
  VALUES 
      ('THE RITZ LONDON', 300),
  ...
  if (!existePacientes) {
    await db.run(`CREATE table pacientes( 
    ...
    await db.run(`INSERT INTO pacientes (ApeNomPaciente, NroHCPaciente,DomicilioPaciente)
    VALUES 
    ('LOPEZ, JUAN', 12345, 'AVENIDA SANTA FE 123'),
    ('GOMEZ, MARIA', 23456, 'CALLE MITRE 456'),
 ('RODRIGUEZ, CARLOS', 34567, 'AVENIDA CORDOBA 789'),
 ('MARTINEZ, LAURA', 45678, 'CALLE SUIPACHA 987'),m
